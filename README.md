...
- 规划层（`probe.ts`）把补测集合有效性归约为对各替代矩阵差异单元集合的**集合覆盖**：
  ...
  的记忆化可行性判定，构造该测点数下**行优先字典序最小**的有效集合；
- 10 点仍不足时（仅在替代矩阵数 `<200`、已完整枚举时）：对“至多 10 点能覆盖多少替代矩阵”
  做**精确**最优化（二分目标覆盖数 + 记忆化可行性搜索，辅以两两不相交见证的打包上界与
  覆盖函数单调子模的贪心序列上/下界，剪枝均为有效界，不设近似节点预算），再逐位置取仍能
  达到该最大覆盖数的最小坐标，得到覆盖数最大、并列时行优先字典序最小的努力计划，
  并给出首个未分辨矩阵；
