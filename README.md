...
- 计划逐点列出预期圈数，并给出**移除该点后首个重新无法区分的替代见证**（该点为冗余点
  时则注明移除后仍可全部分辨）；
- 若当前结论本已唯一，明确提示**无需补测**；
- 若 10 个测点仍不能区分全部替代矩阵，给出**完整比较全部至多 10 点组合后**
  覆盖替代矩阵数最多的努力计划（非启发式近似）；覆盖数并列时取测点行优先
  坐标序列字典序最小者，并给出与该计划一致的**首个未被区分的圈数矩阵与
  其差异单元**；
...
- 规划层（`probe.ts`）把补测集合有效性归约为对各替代矩阵差异单元集合的**集合覆盖**：
  ...
  的记忆化可行性判定，构造该测点数下**行优先字典序最小**的有效集合；10 点不足时按
  固定测点数（至多 10 个差异点，不足 2 点再用最小可触达格填充）做精确分支限界——
  以“后缀差异点覆盖并集”“剩余名额 × 单点最大新增覆盖”两条不高估的上界剪枝，
  配合贪心可行解下界与字典序持平分枝判定，完整比较后取覆盖最多、并列时字典序
  最小的努力计划，并给出与该计划一致的首个未分辨矩阵；
